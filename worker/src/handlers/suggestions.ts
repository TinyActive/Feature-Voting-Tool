...
import {
  sendEmail,
  generateSuggestionApprovedEmail,
  generateSuggestionRejectedEmail,
} from "../utils/email";
...
async function getSuggestionById(
  ...
  const suggestion = await env.DB.prepare(
    "SELECT * FROM feature_suggestions WHERE id = ?",
  )
    .bind(suggestionId)
    .first();
  ...
  if (!suggestion) {
    return {
      suggestion: null,
      error: jsonResponse({ error: "Suggestion not found" }, 404),
    };
  }
 ...
 * Headers: Authorization: Bearer <token>
 */
export async function handleGetMySuggestions(
  request: Request,
  env: Env,
): Promise<Response> {
  try {
 ...
 */
export async function handleGetAllSuggestions(
  request: Request,
  env: Env,
): Promise<Response> {
 ...
 * Headers: Authorization: Bearer <admin-token>
 */
export async function handleApproveSuggestion(
  request: Request,
  env: Env,
): Promise<Response> {
  const authError = await verifyAdmin(request, env);
  ...
  try {
    ...
    const { suggestion, error: fetchError } = await getSuggestionById(
      env,
      suggestionId!,
    );
